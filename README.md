...
Every `branch` case must label at least one outbound edge out of that node; a case that should end the run points at a terminal node rather than being left unrouted. Validation refuses the graph otherwise, naming the node and the case. The mirror image is refused too: an outbound edge from a `branch` node labeled with a name none of its cases declare is dead (it can never fire), and validation names the node and the label. An outbound edge from a `branch` node with no label at all can never fire either, for the same reason, and is refused just as precisely, naming the node and the edge's target.
...
The showcase is [`examples/payroll/`](examples/payroll/): a payroll run that fans a `map` out over twelve employees, parks at a human gate when the batch looks wrong, gets `kill -9`ed mid-batch with four paid, and recovers with every employee paid exactly once at the amounts the approver signed. `bash examples/payroll/run.sh` proves it offline, no key and no network. Four more carry the detail: [`examples/graphs/`](examples/graphs/) is the documents and the builders, [`examples/graph-service/`](examples/graph-service/) drives a refund dispute end to end from the CLI with a mid-run kill, [`examples/graph-clients/`](examples/graph-clients/) drives that same desk from Python, TypeScript and Rust application code over a stock `salvor serve`, and [`examples/follow-up/`](examples/follow-up/) is a graph with a `delay` node, driven back to completion offline by `bash examples/follow-up/run.sh`, no key needed.
...
`salvor serve` also sweeps for due timers every 60 seconds by default (`--wake-interval` in seconds, `0` turns the sweep off), but by the hash a run recorded: an agent run wakes once that agent is registered with `POST /v1/agents`, MCP tools and all, and a graph run wakes once its document is submitted with `POST /v1/graphs` and every `tool` node in it names a tool this server's own registry holds (empty by default). A run whose hash is not registered here, typically one started from the CLI against a store this server never saw, or a graph run with a `tool` node this registry does not hold, stays asleep until `salvor wake` drives it with the same `--agent`/`--graph` files (see [`docs/OPERATIONS.md#waking-sleeping-runs`](docs/OPERATIONS.md#waking-sleeping-runs)). Resuming a sleeping run before its `wake_at` is refused with the time remaining, on the CLI and as `409 still_sleeping` over HTTP, rather than silently doing nothing.
...
The client fetches the declarations from `GET /v1/client-tools`, opens an intent whose input the server checks against the declared schema and records before anything happens, performs the call in its own process with its own credentials, and reports the result, which the server checks against the declared output schema before it becomes part of the record. The idempotency key is derived by the server, never accepted from the caller, so an honest retry gets the identical key and a duplicate cannot be smuggled in by presenting a fresh one. What it is derived from is the operator's too: from the run, the position, and the tool name by default, which identifies an attempt, or from `idempotency_key = ["order_id", "amount_cents"]`, which identifies the call, so the same refund asked for twice in one run is answered from the first rather than issued again. A declaration silent about trust may not self-complete: `trust_completion` defaults to `false`, every call settles by hand through the existing resolve endpoint, and `trust_completion = true` is the explicit opt-in for a tool whose report the operator will take. Even then the output schema checks shape, not truth: an intent for 5000 cents would accept a well-shaped completion claiming 50000. `require_equal = ["amount_cents"]` closes that gap, refusing any report whose named fields differ from what the intent recorded, and a refused or absent completion leaves the intent as the log's last word, so the run derives to needs-reconciliation until a person settles it. A call that failed outright reports `{"error": {"message": ...}}` instead of an output, and salvor records the same failure a native tool records. A person settling a call by hand meets the same declaration the client would have, and the completion they record says an operator recorded it. Worked end to end in [`examples/client-tools/`](examples/client-tools/).
