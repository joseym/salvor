...
`salvor serve` also sweeps for due timers every 60 seconds by default (`--wake-interval` in seconds, `0` turns the sweep off), and `salvor wake` runs one sweep from cron for a store no server is watching. Resuming a sleeping run before its `wake_at` is refused with the time remaining, on the CLI and as `409 still_sleeping` over HTTP, rather than silently doing nothing.
