/**
 * THE ONE JSON HIGHLIGHTER for this feature — ported from the OD prototype's `jsonHi` (hand-rolled,
 * no library). Every pretty-printed JSON site inside the Inbox (the reconciliation card's evidence
 * `<dd>`, the evidence panel's `<pre>` blocks) routes through this, so highlighting is everywhere or
 * nowhere within the feature.
 *
 * DIVERGENCE, filed: the Inspector ports the SAME algorithm independently in its own feature
 * directory, because the two features landed in parallel with no shared-utility landing zone
 * yet. Two copies of one small pure
 * function, not a shared abstraction — a sync item for a later consolidation pass (`core/` or a
 * shared `ui/` module), not a defect in either feature.
 *
 * SAFETY, unchanged from the prototype's own invariants (both asserted by spec 10's escape-safety
 * sweep, though that spec is not in this file's own gate):
...
/**
 * Pretty-print `value` at `indent` spaces (0/undefined for compact single-line, matching the
 * prototype's evidence-`<dd>` call sites) with each token wrapped in a `span.j-key` / `.j-str` /
