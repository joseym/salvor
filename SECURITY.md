...
Findings that amount to "the documented posture is permissive" (no multi-tenancy, unencrypted store, trusted native tools, no external anchor over the hash chain) are not vulnerabilities, though a suggestion for how to tighten them is welcome as an issue.
