...
|---|---|---|
| `agent` | `agent_hash` (a `sha256:<64 hex>` string), optional `input_schema` / `output_schema` | A full agent loop, referenced BY CONTENT HASH, never an embedded definition. An `output_schema` here has a RUNTIME meaning as well as a documentary one: the engine runs that node's loop in structured mode. The model is offered a synthetic `salvor_answer` tool whose input schema IS the declared schema, and the request requires it to call some tool, so the reply is delivered through that call and the node's output is the call's validated input rather than the reply text. The check is this repo's own structural validator, which honors `type`, `required`, `properties`, `items`, `enum`, the numeric bounds (`minimum`, `maximum`, `exclusiveMinimum`, `exclusiveMaximum`), `minLength`/`maxLength` and `minItems`/`maxItems`, and never rejects a value over a keyword outside that set (`pattern`, `format`, `oneOf`, and the rest are read but not enforced). An answer that fails the check goes back to the model with the violation named, and the loop asks again until the steps budget stops it. The same field is separately used at load time, on the document, for the edge type-compatibility check below; the two uses do not interact. |
...
| `delay` | `seconds` | A durable wait. The engine records `NodeEntered`, reads the clock into the log, records `SleepStarted` with the instant it derives from that reading, and parks the run. Before the deadline a drive records NOTHING and the node stays entered with no `NodeExited`, so a waker (`salvor wake`, or `serve`'s sweeper) simply re-drives it; at or past the deadline `SleepCompleted` lands, the node exits, and the walk continues. The wait is a DURATION and never an absolute instant: a document is content-addressed and run any number of times, so a baked-in wake time would be right on the first run and already past on the second, while a duration says the thing that stays true and lets the engine derive the instant per run from a recorded reading. The node transforms nothing, so its OUTPUT IS ITS INPUT VERBATIM; it moves a run in time, never in value. A wait of zero is refused at validate: the node's whole meaning is the wait, so a wait of nothing is a mistake rather than an intent, and deleting the node already says it. No event kind of its own: `SleepStarted` / `SleepCompleted` is the whole vocabulary, exactly as `Suspended` / `Resumed` is the gate's. |
...
| [`delay-then-publish.json`](delay-then-publish.json) | A `tool` scores a draft, a `delay` holds the run for an hour, a `tool` publishes what the delay handed on unchanged. The shared fixture the Rust, TypeScript, and Python builders all reduce to for a `delay` node. |
...
A `delay` node parks the run on a timer instead of on a person: nothing to
supply, so `salvor wake --graph <FILE>` (or a running `salvor serve`, which
sweeps due runs itself) drives it on once the deadline has passed, and a drive
that arrives early records nothing. `graph run` prints that command when a run
parks on a timer.
...
- **Every `branch` case names an outbound edge.** A case with no labeled edge
  ...
  is refused naming the node and the label. So is an outbound edge from a
  `branch` node with no label at all, for the same reason, naming the node
  and the edge's target.
