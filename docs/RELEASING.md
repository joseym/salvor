...
workflows that fire on the same tag. Six releases have shipped through it so
far: v0.5.0, v0.5.1, v0.5.2, v0.5.3, v0.6.0, and the current v0.7.0. Pushing a
`v*` tag builds the `salvor` binary for every target, publishes the crate
family to crates.io, the Python SDK to PyPI, the TypeScript SDK and the CLI
installer to npm, publishes the container image to `ghcr.io/joseym/salvor`,
and creates the GitHub Release with the binaries attached. One channel is not
live yet, the Homebrew tap; see "Prerequisites for each release channel"
below for what it's missing.
...
3. The push of a `v*` tag triggers five workflows, all matching the same tag
   pattern:
   ...
   - `.github/workflows/docker.yml` builds `ghcr.io/joseym/salvor` and
     publishes it for `linux/amd64` and `linux/arm64` on the same tag. It
     first published on the `v0.6.0` tag; see "Container image" under
     Prerequisites for what's live today.
...
different job, different artifact, different registry. It first published an
image on the `v0.6.0` tag; see "Container image" under Prerequisites for what
that publish included. See [CONTAINER.md](CONTAINER.md) for how to run the
image.
...
today at 0.7.0, including `salvor`, `salvor-cli`, `salvor-core`, and
`salvor-runtime`.
...
same tag is safe. `salvor` on PyPI is live today at 0.7.0.
...
- `.github/workflows/docker.yml` builds `ghcr.io/joseym/salvor` on pushes to
  ...
  `v*` tag as the rest of this section. It first published on the `v0.6.0`
  tag.
- It uses the repository's built-in `GITHUB_TOKEN` (granted `packages:
  write`); no separate registry credential is needed.

`ghcr.io/joseym/salvor` is live today with `0.6.0`, `0.7.0`, and `latest`
tags, all genuinely multi-architecture. See [CONTAINER.md](CONTAINER.md) for
...
- `v0.5.0` through `v0.7.0` have each shipped a GitHub Release with archives,
...
- `ghcr.io/joseym/salvor` is on the GitHub Container Registry with `0.6.0`,
  `0.7.0`, and `latest` tags, published for `linux/amd64` and `linux/arm64`.
- `dist-workspace.toml` now builds seven targets: `aarch64-apple-darwin`,
  ...
  v0.5.3 shipped and first released in v0.6.0.
